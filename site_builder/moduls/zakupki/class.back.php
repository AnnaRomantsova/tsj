...
//         $sub->addField('city', $r->result('city'));
         ...
         $rs = new Select($_this->db,'select c.name from zakupki z, house h, city c,street s  where z.id_house=h.id and c.id=s.id_city and h.id_street=s.id and z.id= '.$r->result('id'));
         if ($rs->next_row())
         ...
         $sub->addField('date_begin1',date('d.m.Y', $r->result('date_begin')));
